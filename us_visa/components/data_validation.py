...
class DataValidation:
    ...
    def detect_dataset_drift(self, reference_df: DataFrame, current_df: DataFrame, ) -> bool:
        ...
        try:
            ...
            json_report = json.loads(snapshot.json()) if hasattr(snapshot, "json") else snapshot.dict()
            ...
            for metric in json_report.get("metrics", []):
                if metric.get("metric_name", "").startswith("DriftedColumnsCount"):
                    n_drifted_features = int(metric.get("value", {}).get("count", 0))
                    drift_status = metric.get("value", {}).get("share", 0.0) >= 0.5
                    break
